...
path: STRING {identify_path($1); cout<<"\n\tSTRING\t";}
   | STRING LP RP {identify_fun_path($1);}
   | STRING AT STRING  {identify_path($1,"@",$3); cout<<"\n\tAT\t";}
   | path ANY path {add_any(); cout<<"\n\tANY\t";}
   | path CONCAT path {add_concat(); cout<<"\n\tCONCAT\t";}
   ;
  
logicalExpr: STRING logicalOp INT {new_expr($1, $3); }
	   | STRING LP RP logicalOp INT{new_fun_expr($1, $5);}
	   | STRING LP RP logicalOp TRUE{new_fun_expr($1);}
	   | STRING LP RP logicalOp FALSE{new_fun_expr($1);}
	   | STRING logicalOp TRUE 	{ new_expr($1, "1"); }
	   | STRING logicalOp FALSE { new_expr($1, "0"); }
