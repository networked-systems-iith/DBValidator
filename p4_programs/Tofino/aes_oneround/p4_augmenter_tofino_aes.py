...
def augmenter(p4_filename, table_to_action, weighted_edges):
    ...
    index_metadata = new_data.find('{', new_data.find('struct ig_metadata_t')) + 1
    ...
    for we in weighted_edges:
        ...
        if we['weight'] > 0:
            ...
            if we['src'] in table_to_action.keys() and 'tbl_' not in we['src']:
                ...
                if new_data.find(search_string.encode('ascii', 'ignore')) != -1:
                    req_ind = new_data.find('{', new_data.find(search_string.encode('ascii', 'ignore'))) + 1
...
for n in nodes:
	if n not in actual_nodes:
		to_del.append(n)
final_relative_match_node =[]

for d in to_del:
  if d == 'hdr.aes_meta.isValid()'or d== "hdr.aes.isValid()" or d == "tbl_drop":
    final_relative_match_node.append(d)
  nodes.remove(d)
nodes = nodes + action+ final_relative_match_node
#print(nodes)
edge_to_del = []
rel_edge = edges
for e in rel_edge:
